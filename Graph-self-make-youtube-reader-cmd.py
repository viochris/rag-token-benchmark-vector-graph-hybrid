...
# LlamaIndex core components for document modeling, graph indexing, and application context
...
# LlamaIndex integrations for specific external services (Google GenAI, LiteLLM, Neo4j)
...
# ==============================================================================
# 2. HELPER FUNCTION: YOUTUBE TRANSCRIPT EXTRACTOR
# ==============================================================================
    ...
    for url in youtube_urls:
        ...
        if "youtube" in url:
            ...
            if "v" in parse_qs(parsed_url.query):
                ...
                raise ValueError(f"🚨 [SYSTEM ERROR] Invalid YouTube URL format: {url}")
        ...
        documents.append(Document(text=full_text.strip(), metadata={"video_id": video_id, "video_url": url}))
...
try:
    ...
    tracer_provider = register(
        project_name="youtube_graph_project",
    ...
    # ==============================================================================
    # 5. KNOWLEDGE GRAPH INGESTION OR RETRIEVAL
    # ==============================================================================
    ...
    if not graph_has_data:
        print("⏳ [SYSTEM] Databases are empty. Initializing data ingestion and graph extraction...")
        
        # Load and parse transcripts from the provided YouTube URLs.
        documents = load_youtube_transcripts(youtube_urls)
        
        # Configure the LLM path extractor to dictate the structural extraction limits.
        ...
        print("🧠 [SYSTEM] Extracting Property Graph (Nodes & Triplets) using LLM...")
        ...
        print("✅ [SYSTEM] Property Graph successfully created and stored.")
    else:
        print("📂 [SYSTEM] Existing data found. Loading existing index...")
        
        # Reconstruct the index mapping directly from the persisted Neo4j database.
        index = PropertyGraphIndex.from_existing(
            property_graph_store=graph_store,
            llm=Settings.llm
        )
        print("✅ [SYSTEM] Index successfully loaded from existing storage.")

    # ==============================================================================
    # 6. CHAT ENGINE & MEMORY SETUP
    # ==============================================================================
    print("🧠 [SYSTEM] Setting up Chat Engine with PostgreSQL memory...")
    
    # Establish a persistent conversational memory backend using AsyncPG to maintain user context.
    memory = Memory.from_defaults(
        session_id="youtube_graph_session",
        token_limit=40000,
        async_database_uri=os.getenv("ASYNCPG_DATABASE_URL"),
        table_name="youtube_graph_memory"
    )

    # Define the operational boundaries and strict behavioral constraints for the AI Agent.
    ai_system_prompt = (
        "You are an intelligent knowledge graph assistant specializing in analyzing YouTube video content. "
        "Your primary objective is to answer user queries strictly based on the provided context retrieved from the database. "
    ...
    async def main():
        ...
        # ==============================================================================
        print("\n" + "="*70)
        print("💬 [SYSTEM] Interactive Graph Agent Session Started. Type 'exit' to stop.")
        print("="*70)
        ...
        while True:
            ...
            # Execute the asynchronous query utilizing the graph context retrieval.
            print("🧠 [AI] Querying the knowledge graph...")
            ...
            print(f"\n🔍 [SOURCE NODES] Retrieved {len(source_nodes)} relevant nodes from the graph:")
    ...
    # Execute the asynchronous event loop for the chat engine.
